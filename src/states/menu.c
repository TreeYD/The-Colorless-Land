...
double GetAnimationX(double staX, int x) {
	return staX + 0.95*x*0.01042;
}
double GetAnimationY(double staY, int y) {
	int up = 110;
	int a = y / up, b = y % up;
	if (!(a & 1))return staY - 0.5*b*INCHRATE;
	return staY - 0.5*INCHRATE*up + 0.5*b*INCHRATE;
...
void drawMainMenu()
{
	...
	if (animation_line <= ANIMATIONSTAGE && animation_line >= 0) {
		//display logo
		DisplayClear();
		AddZoomBitMap("logo.bmp", 3.5, 4.6, 8, 3.14, SRCCOPY);
	...
	if (animation_line > 800) {
		double L = INCHRATE *420;
		double H = INCHRATE*250;
		BUTTON newGame;
